...
def dump_db_to_sql(dir_processing_azm):
    ...
    cmd = [
        ...
        ".schema" if (
            is_dump_schema_only_for_target_db_type(args)) else ".dump"
    ]
...
def handle_sql3_dump_line(args, line):
    ...
    if line.startswith("CREATE TABLE ") or g_is_in_create:
        ...
        dprint("is this table in --omit_tables ? " +
               table_name + " = " + str(is_omit_table))

        if args['only_tables_on']:
            dprint(
                "--only_tables on - check if we should exclude this table: " + table_name)
            is_omit_table = True
            if table_name in args['only_tables_array']:
                is_omit_table = False
            dprint("--only_tables on - exclude this table? " +
                   table_name + " = " + str(is_omit_table))
    ...
    if (
        ...
        if not args['unmerge']:
            ...
            #print "call cmd:", cmd
            try:
                outstr = subprocess.check_output(cmd, text=True)
        ...
        # always create - flag override
        print("processing create at handler module...")
...
def unzip_azm_to_tmp_folder(args):
    ...
    if os.path.isfile(azm_fp):
        ...
        raise Exception(
            "INVALID: - azm file does not exist at given path: " + str(azm_fp) + " - ABORT")
    ...
    dir_processing_azm = os.path.join(dir_azm_unpack, "tmp_azm_db_merge_" + str(
        uuid.uuid4()) + "_" + azm_name_no_ext.replace(" ", "-"))  # replace 'space' in azm file name
    ...
    try:
        ...
        try:
            ...
            if integ_check_df.iloc[0].integrity_check == "ok":
                ...
                dump_ret = subprocess.call("sqlite3 '{}' .dump > '{}.txt'".format(dbfile, dbfile),shell=True, text=True)
                ...
                if dump_ret != 0:
                    ...
                    os.remove(dbfile)
                    import_ret = subprocess.call("sqlite3 '{}' < '{}.txt'".format(dbfile, dbfile), shell=True, text=True)
        ...
        if args['get_schema_shasum_and_exit']:
            ...
            print("call cmd:", cmd)
            schema = subprocess.check_output(cmd, text=True)
            # print "get_schema_shasum_and_exit 3"
            sha1.update(schema.encode())
            # print "get_schema_shasum_and_exit 4"
            print(str(sha1.hexdigest(
            )) + " is the sha1 for the schema of azqdata.db inside azm: " + args['azm_file'])
        ...
        try:
            ...
            pass
        raise Exception(
            "Invalid azm_file: azm file does not contain azqdata.db database - exception: " + str(e))
...
def cleanup_tmp_dir(dir_processing_azm):
    ...
    if dir_processing_azm is not None and os.path.exists(dir_processing_azm) and os.path.isdir(dir_processing_azm):
        pass
    else:
        return
    for i in attempts:
        try:
            ...
            print("warning: attempt %d/%d - failed to delete tmp dir: %s - dir_processing_azm: %s" %
                  (i, imax, e, dir_processing_azm))
...
def check_azm_azq_app_version(args):
    ...
    MIN_APP_V1 = 0
    MIN_APP_V2 = 587
    # there is always only 1 ver of AZENQOS app for 1 azm - and normally 1 row of logs per azm too - but limit just in-case to be future-proof
    sqlstr = "select log_app_version from logs"
    cmd = [args['sqlite3_executable'], args['file'], sqlstr]
    print("call cmd:", cmd)
    outstr = subprocess.check_output(cmd, text=True).strip()
    try:
        args["azm_apk_version"] = "0.0.0"
        # replace 'v' prefix - like "v3.0.562" outstr
        ...
        if (args["azm_apk_version"] >= MIN_APP_V0 * 1000 * 1000 + MIN_APP_V1 * 1000 + MIN_APP_V2):
            ...
            print("WARNING: azm too old - azm file must be from AZENQOS apps with versions {}.{}.{} or newer.".format(
                MIN_APP_V0, MIN_APP_V1, MIN_APP_V2))
    except Exception:
        type_, value_, traceback_ = sys.exc_info()
        exstr = traceback.format_exception(type_, value_, traceback_)
...
def mv_azm_to_target_folder(args):
    ...
    if mv_target_folder is not None:
        ...
        try:
            ...
            pass
        print("move_imported_azm_files_to_folder: mv {} to {}".format(
            azm_fp, target_fp))
...
def process_azm_file(args):
    ...
    try:
        ...
        if dry_mode:
            print(
                "dry_mode - dont unzip azm for azqdata.db - let preprocess func handle itself")
        else:
        ...
        if args['target_db_type'] == "sqlite3":
            ...
            print(
                "sqlite3 - import to {} from {}".format(args['target_sqlite3_file'], dumped_sql_fp))
        ...
        ori_log_hash_datetime = datetime.fromtimestamp(
            args['log_hash'] & 0xffffffff)  # log_hash lower 32 bits is the timestamp
        ...
        args['log_end_time'] = datetime.fromtimestamp(
            int(args['log_end_time']))
        print("args['log_end_time']:", args['log_end_time'])
        print("args['log_end_time_str']:", args['log_end_time_str'])

        args['log_data_min_time'] = args['log_start_time'] - \
            timedelta(hours=48)
        ...
        if mv_target_folder is not None and not os.path.exists(mv_target_folder):
            os.makedirs(mv_target_folder)
        if mv_target_folder is not None:
            azm_fp = os.path.abspath(args['azm_file'])
            target_fp = os.path.join(
                mv_target_folder, os.path.basename(azm_fp))
            try:
                os.remove(target_fp)
                os.remove(target_fp + "_output.txt")
            except Exception:
...
if __name__ == '__main__':
    ...
    try:
        ...
        if ret == 0:
            ...
            raise Exception(
                "Secified (or default) --sqlite3_executable not working - ABORT")
    except Exception as e:
        estr = str(e)
        ...
        if "The system cannot find the file specified" in estr:
            ...
            if ret == 0:
                ...
                raise Exception(
                    "Failed to find sqlite3 - please install sqlite3 and make sure it is in the path first. exception:" + str(e))
    ...
    if not args['move_imported_azm_files_to_folder'] is None:
        ...
        if os.path.isdir(args['move_imported_azm_files_to_folder']):
            ...
            raise Exception("ABORT: Can't create or access folder specified by --move_imported_azm_files_to_folder: " +
                            str(args['move_imported_azm_files_to_folder']))
    ...
    g_check_if_already_merged_function = getattr(
        mod, 'check_if_already_merged')
    ...
    if "," in args['azm_file']:
        print(
            "found comman in args['azm_file'] - split and use whichever is first present in the list")
        csv = args['azm_file']
        found = False
        for fp in csv.split(","):
        ...
        if not found:
            raise Exception(
                "Failed to find any valid existing azm file in supplied comma separated --azm_file option:" + str(args['azm_file']))
    ...
    if folder_daemon:
        if not azm_file_is_folder:
            raise Exception(
                "ABORT: --daemon_mode_rerun_on_folder_after_seconds specified but --azm_file is not a folder.")
        folder_daemon_wait_seconds = int(
            args['daemon_mode_rerun_on_folder_after_seconds'])
        print("folder_daemon_wait_seconds: ", folder_daemon_wait_seconds)
        if folder_daemon_wait_seconds <= 0:
            raise Exception(
                "ABORT: --daemon_mode_rerun_on_folder_after_seconds option must be greater than 0.")
    ori_args = args
    ...
    if args['add_imei_id_to_all_tables']:
        ...
        sqlstr = "select {} from {} {} order by seqid desc limit 1;".format(
            col, table, where)
        cmd = [args['sqlite3_executable'], args['file'], sqlstr]
        print("call cmd:", cmd)
        imei = subprocess.check_output(cmd, text=True).strip()
        args['imei'] = imei
    ...
    while(True):
        ...
        # args gets modified by each run - especially ['azm_file'] gets changed - so we want to use a copy of the original_args here (otherwise args would get modified and we won't be able to restore to the original for daemon_mon
        args = ori_args.copy()
        ...
        if azm_file_is_folder:
            dir = args['azm_file']
            print("supplied --azm_file: ", dir,
                  " is a directory - get a list of .azm files to process:")
            matches = []
        ...
        for azm in azm_files:
            ...
            try:
                ...
                if (ret != 0):
                    raise Exception(
                        "ABORT: process_azm_file failed with ret code: " + str(ret))
                print(
                    "## DONE process azm {}/{}: '{}' retcode {}".format(iazm, nazm, azm, ret))
            except Exception as e:
                ifailed = ifailed + 1
                had_errors = True
                type_, value_, traceback_ = sys.exc_info()
                exstr = traceback.format_exception(type_, value_, traceback_)
                print("## FAILED: process azm {} failed with below exception:\n(start of exception)\n{}\n{}(end of exception)".format(
                    azm, str(e), exstr))
        ...
        if (had_errors is False):
            print("SUCCESS - operation completed successfully for all azm files (tatal: %d) - in %.03f seconds." %
                  (iazm, time.time() - process_start_time))
        else:
            print("COMPLETED WITH ERRORS - operation completed but had encountered errors (tatal: %d, failed: %d) - in %.03f seconds - (use --folder_mode_stop_on_first_failure to stop on first failed azm file)." %
                  (iazm, ifailed, time.time() - process_start_time))
        if not folder_daemon:
            print("exit code:", str(ret))
            exit(ret)
        else:
            print("*** folder_daemon mode: wait seconds: ",
                  folder_daemon_wait_seconds)
            for i in range(0, folder_daemon_wait_seconds):
                print("** folder_daemon mode: waiting ", i, "/",
                      folder_daemon_wait_seconds, " seconds")
                time.sleep(1)
