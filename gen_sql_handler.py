...
def connect(args):
    ...
    if g_is_postgre and not args['unmerge']:
        ...
        # create postgis at public schema first
        try_cre_postgis(schema="public")
        ...
        if args["pg_schema"] != "public":
            ...
            try:
                with g_conn as c:
                    g_cursor.execute(
                        "create schema if not exists " + args["pg_schema"])
                    c.commit()
                    print("success: create schema " +
                          args["pg_schema"] + " success")
...
def try_cre_postgis(schema="public"):
    ...
    try:
        with g_conn as c:
            sql = "CREATE EXTENSION if not exists postgis SCHEMA {}".format(
                schema)
        ...
        if 'already exists' in estr:
            ...
            print(
                "FATAL: CREATE EXTENSION postgis - failed - please make sure postgis is correctly installed.")
            raise e
...
def check_if_already_merged(args, log_hash):
    ...
    if args["pg_schema"] != "public":
        g_cursor.execute(
            "SET search_path = '{}','public';".format(args["pg_schema"]))
    ...
    try:
        ...
        if (row is None):
            ...
            if (args['unmerge']):
                ...
                print(
                    "### unmerge mode - delete start for azm: log_hash {}".format(row['log_hash']))
                ...
                raise Exception(
                    "ABORT: This log ({}) has already been imported/exists in target db (use --unmerge to remove first if you want to re-import).".format(log_hash))
        ...
        if ("Invalid object name 'logs'" in estr or '42S02' in estr
            ...
            print(
                "looks like this is the first-time log import - no table named logs exists yet - ok...")
            if args['unmerge']:
                raise Exception(
...
def commit(args, line):
    ...
    for buf in g_exec_buf:
        if isinstance(buf, tuple):
            # for COPY from stdin
            buf, dump_fp = buf
            with open(dump_fp, "r") as dump_fp_fo:
            ...
            try:
                ...
                if "does not exist" in str(e) and args['unmerge']:
                    print(
                        "WARNING: unmerge exception: {} - but ok for --umnerge mode if exec delete and face - does not exist exception...".format(e))
                else:
                    raise e
    ...
    if args['dump_parquet']:
        ...
        if args['unmerge']:
            ...
            if rmcmdret != 0:
                raise Exception(
                    "Remove files from object store failed cmcmdret: {}".format(rmcmdret))
            try:
                with g_conn:
                    update_sql = "update uploaded_logs set non_azm_object_size_bytes = null where log_hash = {};".format(
                        args['log_hash'])
                    print("update_sql:", update_sql)
                    g_cursor.execute(update_sql)
            except Exception:
                type_, value_, traceback_ = sys.exc_info()
                exstr = str(traceback.format_exception(
                    type_, value_, traceback_))
                print(
                    "WARNING: update uploaded_logs set non_azm_object_size_bytes to null failed exception:", exstr)
            ...
            if cpcmdret != 0:
                raise Exception(
                    "Copy files to object store failed cmcmdret: {}".format(cpcmdret))
            try:
                combined_pq_size = 0
                ...
                with g_conn:
                    update_sql = "update uploaded_logs set non_azm_object_size_bytes = {} where log_hash = {};".format(
                        combined_pq_size, args['log_hash'])
                    print("update_sql:", update_sql)
                    g_cursor.execute(update_sql)
            except Exception:
                type_, value_, traceback_ = sys.exc_info()
                exstr = str(traceback.format_exception(
                    type_, value_, traceback_))
                print(
                    "WARNING: update uploaded_logs set non_azm_object_size_bytes to parquets size failed exception:", exstr)
...
def create(args, line):
    ...
    if table_name == "logs":
        uline = line.replace('"log_hash" BIGINT,',
                             '"log_hash" BIGINT UNIQUE,', 1)
    ...
    if table_name == "wifi_scanned":
        ...
        print("check azm apk ver for wifi_scanned table omit: ",
              args["azm_apk_version"])
    ...
    if (g_unmerge_logs_row is not None):
        ...
        sqlstr = "delete from \"" + table_name + \
            "\" where \"log_hash\" = {}".format(g_unmerge_logs_row['log_hash'])
        g_exec_buf.append(sqlstr)
    ...
    for lsp in ls:
        ...
        if len(splitted) < 3:
            raise Exception(
                "failed to get col_name/col_type for lsp: {}".format(lsp))
        col_name = splitted[1]
    ...
    if (not args['dump_parquet']) or (table_name == "logs"):
        try:
            ...
            if g_is_postgre:
                ...
                if args['pg10_partition_by_month']:
                    if table_name == "logs":
                        ...
                        try:
                            with g_conn:
                                g_cursor.execute("SELECT schema_name FROM information_schema.schemata WHERE schema_name = '{}';".format(
                                    schema_per_month_name))
                                if bool(g_cursor.rowcount):
                                    print(
                                        "schema_per_month_name already exists:", schema_per_month_name)
                                    pass
                                else:
                                    print(
                                        "cre schema now because: NOT schema_per_month_name already exists:", schema_per_month_name)
                                    with g_conn:
                                        ...
                                        g_conn.commit()
                                        print(
                                            "success: create per_month [" + c_table_per_month_sql + "] success")
                        except Exception:
                            type_, value_, traceback_ = sys.exc_info()
                            exstr = str(traceback.format_exception(
                                type_, value_, traceback_))
                            print(
                                "WARNING: create table_per_month schema failed - next insert/COPY commands would likely faile now - exstr:", exstr)
            ...
            if g_is_postgre:
                with g_conn:
                    g_cursor.execute(
                        "select * from information_schema.tables where table_schema=%s and table_name=%s", (args["pg_schema"], table_name,))
                    if bool(g_cursor.rowcount):
                        # print "omit already existing table - raise exception to check columns instead"
                        raise Exception(
                            "table {} already exists - no need to create".format(table_name))
            ...
            if ("There is already an object named" in emsg or
                ...
                if (len(remote_columns) == 0):
                    raise Exception(
                        "FATAL: failed to parse/list remote columns")
                ...
                if (n_cols_to_add == 0):
                    ...
                    print("n_cols_to_add: " + str(n_cols_to_add) + " - need to alter table - add cols:" +
                          str(local_columns_not_in_remote) + "\nremote_cols:\n" + str(remote_columns))
    ...
    if g_bulk_insert_mode:
        ...
        if args['pg10_partition_by_month'] and not args['dump_parquet']:
            if table_name == "logs":
                ...
                months_pt_check_list = [ori_log_hash_datetime + relativedelta(
                    months=-1), ori_log_hash_datetime, ori_log_hash_datetime + relativedelta(months=+1)]

                for pre_post_month_log_hash_datetime in months_pt_check_list:
                    log_hash_ym_str = pre_post_month_log_hash_datetime.strftime(
                        '%Y_%m')
                    ...
                    ntn = "logs_{}".format(log_hash_ym_str)
                    ...
                    if per_month_table_already_exists:
                        ...
                        exec_creatept_or_alter_handle_concurrency(
                            cre_target_pt_sql, allow_exstr_list=[" already exists"])

        # let sqlite3 dump contents of table into file

        table_dump_fp = os.path.join(g_dir_processing_azm, table_name + ".csv")
        table_dump_format_fp = os.path.join(
            g_dir_processing_azm, table_name + ".fmt")
        ...
        for col in local_columns:
            ...
            if col_name == "modem_time":
                ...
                # limit to 5 char len (6 is last index excluding)
                col_name = "substr(gsm_bsic, 0, 6) as gsm_bsic"
            elif col_name == "android_cellid_from_cellfile":
                # type cast required to remove non-int in cellfile data
                col_name = "cast(android_cellid_from_cellfile as int) as android_cellid_from_cellfile"
                ...
                if "int" in col_type.lower():
                    print("nr_cell_meas cast to int:  col_name {} col_type {}".format(
                        col_name, col_type))
                    pre = "cast("
                    post = " as int)"
                elif "double" in col_type.lower():
                    print("nr_cell_meas cast to double:  col_name {} col_type {}".format(
                        col_name, col_type))
                    pre = "cast("
                    post = " as double)"
        ...
        if g_is_postgre:
            ...
            if table_name != "logs":
                select_sqlstr += " where time >= '{}' and time <= '{}'".format(
                    args['log_data_min_time'], args['log_data_max_time'])
            ...
            # (not args['dump_parquet']) or parquet_arrow_mode or table_name == "logs":
            if True:
                ret = call(
                    dump_cmd,
                    shell=False
            ...
            print("dump_csv duration:",
                  (datetime.datetime.now() - start_time).total_seconds())

        table_dump_fp_ori = table_dump_fp
        pqfp = table_dump_fp_ori.replace(
            ".csv", "_{}.parquet".format(args['log_hash']))
        ...
        if (not args['dump_parquet']) or (table_name == "logs"):
            ...
            start_time = datetime.datetime.now()
            with open(table_dump_fp, "r") as of:
                ...
                with open(table_dump_fp_adj, "w") as nf:
                    while True:
                        ofl = of.readline()
                        if g_is_postgre:
                            # keep this legacy code for postgres mode code jus to be sure, although we already did nullif checks during sqlite csv dunp...
                            ofl = ofl.replace(',""', ',')
            ...
            print("""find_and_conv_spatialite_blob_to_wkb, replace ,"" with , total file duration:""",
                  (datetime.datetime.now() - start_time).total_seconds())

        # dprint("dump table: "+table_name+" for bulk insert ret: "+str(ret))

        if (ret != 0):
            print("WARNING: dump table: " + table_name +
                  " for bulk insert failed - likely sqlite db file error like: database disk image is malformed. In many cases, data is still correct/complete so continue.")
        ...
        if args['dump_parquet']:
            ...
            for col in list(pa_column_types.keys()):
                ...
                if is_datetime_col(col):
                    ...
                    # some old db invalid type cases
                    pa_column_types[col] = pa.string()
            ...
            for field in cur_schema:
                ...
                if is_datetime_col(field.name):
                    fields_need_pd_datetime.append(
                        pa.field(field.name, pa.timestamp('ns')))
            ...
            if table_name == "signalling":
                ...
                symbol_sr = padf.column(
                    signalling_symbol_column_index).to_pandas().astype(str, copy=False)
                direction_sr = pd.Series(
                    np.zeros(len(symbol_sr), dtype=np.uint8))
            ...
            for i in range(len(fields_need_pd_datetime)):
                ...
                print("converting field index {} name {} to datetime...".format(
                    index, field))
                # convert
                ...
                # assign it back
                # print "padf.schema:\n", padf.schema
                padf = padf.set_column(
                    index, field, pa.Array.from_pandas(converted_sr))
            ...
            if has_geom_field:
                ...
                if not geom_format_in_csv_is_wkb:
                    print(
                        "geom in csv is in spatialite format - convert to wkb first...")
                    spatialite_geom_sr = geom_sr
                    class_type = "01000020E6100000"
                    # spatialite_geom_sr.str.slice(start=2, stop=4)
                    endian = "01"
                    point = spatialite_geom_sr.str.slice(
                        start=86, stop=118)   # 86 + 16 + 16
                    geom_sr = endian + class_type + point  # wkb

                # print 'wkb geom_sr.head():', geom_sr.head()
                geom_sr = geom_sr.str.decode("hex")
                lon_sr = geom_sr.apply(lambda x: None if (x is None or len(
                    x) != WKB_POINT_LAT_LON_BYTES_LEN) else np.frombuffer(x[9:9 + 8], dtype=np.float64)).astype(np.float64)  # X
                lat_sr = geom_sr.apply(lambda x: None if (x is None or len(x) != WKB_POINT_LAT_LON_BYTES_LEN) else np.frombuffer(
                    x[9 + 8:9 + 8 + 8], dtype=np.float64)).astype(np.float64)  # Y
                ...
                padf = padf.set_column(geom_field_index, pa.field(
                    "geom", "binary"), pa.Array.from_pandas(geom_sr))

                # insert lat, lon
                padf = padf.add_column(
                    geom_field_index + 1, pa.field("lat", pa.float64()), pa.Array.from_pandas(lat_sr))
                padf = padf.add_column(
                    geom_field_index + 2, pa.field("lon", pa.float64()), pa.Array.from_pandas(lon_sr))
            ...
            # print "padf.schema:\n", padf.schema
            print("padf processing and conversion with pd duration:",
                  (datetime.datetime.now() - start_time).total_seconds())
            ...
            pq.write_table(padf, pqfp, flavor='spark',
                           compression=PARQUET_COMPRESSION, use_dictionary=True)

            assert os.path.isfile(pqfp)
            print("pq.write_table duration:",
                  (datetime.datetime.now() - start_time).total_seconds())
...
def exec_creatept_or_alter_handle_concurrency(sqlstr, raise_exception_if_fail=True, allow_exstr_list=[]):
    ...
    for retry in range(exec_creatept_or_alter_handle_concurrency_max_retries):
        try:
            ...
            # use with for auto rollback() on g_conn on expected fails like already exists
            with g_conn as con:
                print("exec_creatept_or_alter_handle_concurrency retry {} sqlstr: {}".format(
                    retry, sqlstr))
                execret = g_cursor.execute(sqlstr)
                print("exec_creatept_or_alter_handle_concurrency retry {} sqlstr: {} execret: {}".format(
                    retry, sqlstr, execret))
            ...
            prev_exstr = "WARNING: exec_creatept_or_alter_handle_concurrency retry {} exception: {}".format(
                retry, exstr)
            print(prev_exstr)
    ...
    if ret is False and raise_exception_if_fail:
        raise Exception("exec_creatept_or_alter_handle_concurrency FAILED after max retries: {} prev_exstr: {}".format(
            exec_creatept_or_alter_handle_concurrency_max_retries, prev_exstr))
