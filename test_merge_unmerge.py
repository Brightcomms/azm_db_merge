...
def make_sure_public_logs_doesnt_exist(dbcon, dbcur):
    try:
        with dbcon:
            ...
            raise Exception(
                "INVALID: public.logs should not be created - azm_db_merge is not honoring --pg_schema option")
...
def test():
    ...
    cmd = 'python azm_db_merge.py --unmerge --azm_file "example_logs/" --target_db_type postgresql --server_user {} --server_password {} --server_database {} --pg_schema all_logs | grep "unmerge mode called on an empty database"'.format(
        server_user, server_pass, TMP_DB)
    print("test cmd:", cmd)
    assert 0 == subprocess.call(cmd, shell=True)

    # first import + table creation in schema
    cmd = 'python azm_db_merge.py --azm_file "example_logs/" --target_db_type postgresql --server_user {} --server_password {} --server_database {} --pg_schema all_logs'.format(
        server_user, server_pass, TMP_DB)
    assert 0 == subprocess.call(cmd, shell=True)
    ...
    # unmerge it
    cmd = 'python azm_db_merge.py --azm_file "example_logs/" --target_db_type postgresql --server_user {} --server_password {} --server_database {} --pg_schema all_logs --unmerge'.format(
        server_user, server_pass, TMP_DB)
    assert 0 == subprocess.call(cmd, shell=True)
    dbcur.execute("select * from all_logs.logs")
